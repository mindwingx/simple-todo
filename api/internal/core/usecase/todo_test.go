...
import (
	...
	queueMock "microservice/internal/adapter/queue/mocks"
...
func TestTodoUsecase_Create(t *testing.T) {
	...
	t.Run("successful create with wait-group", func(t *testing.T) {
		...
		uc := NewTodo(logger, locale, queue, todoRepo)
		...
		queue.EXPECT().Send(expectedTodo).DoAndReturn(func(item *domain.Todo) error {
			defer wg.Done()
			return nil
		}).Times(1)
		logger.EXPECT().Info("todo.uc.create.queue.send", gomock.Any()).Times(1)
	...
	t.Run("successful create with sleep for goroutine", func(t *testing.T) {
		...
		todoRepo.EXPECT().Create(ctx, testTodo).Return(expectedTodo, nil).Times(1)
		queue.EXPECT().Send(expectedTodo).Return(nil).Times(1)
		logger.EXPECT().Info("todo.uc.create.queue.send", gomock.Any()).Times(1)
